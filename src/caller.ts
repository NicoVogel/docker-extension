...
export class CustomCaller implements Call {
	...
	invoke(args: string[]): void {
		...
		const execCommand = this.template.replace(
			/\$\d/g,
...
export class Caller implements Call {
	constructor(
...
export const getCallers = (): {
	...
	if (defaultCaller === undefined) {
		...
		defaultCaller = callers.values().next().value;
	}
